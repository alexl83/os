function pre_customize_image__250_3_install_syncthing_repositories() {
	...
	if [[ "${DISTRIBUTION}" == "Debian" ]]; then
		...
		run_host_command_logged echo "deb [arch="${ARCH}" signed-by=/etc/apt/keyrings/syncthing-archive-keyring.gpg] https://apt.syncthing.net/ syncthing stable" "|" tee "${SDCARD}"/etc/apt/sources.list.d/syncthing.list
